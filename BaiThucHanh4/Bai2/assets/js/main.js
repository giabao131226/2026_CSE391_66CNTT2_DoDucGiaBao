...
function checkManhYeu() {
    ...
    if (haveHoa == true || haveThuong == true) {
        inputPassword.classList.remove("alert-error")
        inputPassword.classList.add("border-yellow")
    }
    if (haveHoa == true && haveThuong == true && haveSo == true) {
        inputPassword.classList.remove("border-yellow")
...
inputPassword.addEventListener("keyup", checkManhYeu)
...
const listHienThi = document.querySelectorAll("button.hienThiPassword")
console.log(listHienThi)
listHienThi.forEach((item) => {
    item.addEventListener("click", (e) => {
        const element = e.target.getAttribute("id")
        const tt = e.target.getAttribute("tt")
        if (element == "pass") {
            if (tt == "hidden") {
                e.target.setAttribute("tt", "show")
                inputPassword.setAttribute("type", "text")
            } else {
                e.target.setAttribute("tt", "hidden")
            ...
            if (tt == "hidden") {
                e.target.setAttribute("tt", "show")
                const inputXacNhanMatKhau = document.querySelector("form input[name='xacNhanMatKhau']")
                inputXacNhanMatKhau.setAttribute("type", "text")
            } else {
                e.target.setAttribute("tt", "hidden")
                const inputXacNhanMatKhau = document.querySelector("form input[name='xacNhanMatKhau']")
                inputXacNhanMatKhau.setAttribute("type", "password")
            }
        }
    })
})